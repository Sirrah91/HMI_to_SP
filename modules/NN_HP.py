...
def usage() -> dict[str, str | int | float | bool | list[int]]:
    p = {
        ...
        "num_residuals": 3,  # Number of residual layers
...
def tuning() -> dict:
    p = {
        # kern_size, dropout_hidden_hidden, and input_activation are uniform for all layers;
        # modify similarly to num_nodes if needed
        "model_type": ["CNN", "CNN_sep"],  # Convolutional (CNN); separate parameters (CNN_sep); standard (CNN_classic)
        ...
        "dropout_residual_residual": [0.0, 0.5],  # Dropout rate
