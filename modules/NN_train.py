...
from tensorflow.keras.callbacks import TerminateOnNaN, ReduceLROnPlateau, TensorBoard, ModelCheckpoint
...
from modules.NN_models import MyHyperModel, build_model

from modules.control_plots import plot_model_history, plot_corr_matrix
from modules.utilities_data import (print_header, print_info, load_txt, gimme_used_from_name, unit_to_gauss,
                                    load_keras_model)
...
def train(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray,
    ...
    if model_to_retrain is None:
        ...
        model_name = model_to_retrain.replace("weights", f"weights{_sep_in}retrained")
        ...
        model = load_keras_model(filename=model_to_retrain, input_shape=np.shape(x_train)[1:],
                                 params=params, subfolder=model_subdir)
        model_checkpoint_limit = None  # model.evaluate(x_val, y_val, batch_size=len(y_val), verbose=0)[0]
    ...
    if show_control_plot:
        plot_model_history(model, dt_string=dt_string, quiet=_quiet)

    if not _quiet:
        print_header()
        print_info(unit_to_gauss(y_train, used_quantities),
                   unit_to_gauss(model.predict(x_train, verbose=0), used_quantities),
                   which="train")
        if not is_empty(y_val):
            print_info(unit_to_gauss(y_val, used_quantities),
                       unit_to_gauss(model.predict(x_val, verbose=0), used_quantities),
                       which="validation")
...
def fit_model(model: Model,
    ...
    stopping_patience = np.min((int(params["num_epochs"] * 0.2), 200))
    lr_factor = 1.
...
def train_generator(training_data_generator, validation_data_generator,
                    training_data_shape: tuple[int, ...], validation_data_shape: tuple[int, ...],
                    params: dict[str, str | int | float | bool | list[int]] | None = None,
                    monitoring: dict | None = None,
                    model_subdir: str | None = None,
                    model_name: str | None = None,
    ...
    # Define model architecture
    used_quantities = gimme_used_from_name(model_name)
    model = build_model(input_shape=np.shape(training_data_shape)[1:], params=params, metrics=metrics,
                        used_quantities=used_quantities)
    ...
    model = fit_generator_model(model, training_data_generator, validation_data_generator,
                                training_data_shape[0], validation_data_shape[0],
                                params, monitoring, model_filename, dt_string, verbose)

    save_model(model, model_filename, params=params)

    if show_control_plot:
        plot_model_history(model, dt_string=dt_string, quiet=_quiet)

    """
    # TADY NECO NA ZPUSOB model.evaluate?

    if not _quiet:
        print_header()
        print_info(y_train, model.predict(x_train, verbose=0), which="train")
        if not is_empty(y_val):
            print_info(y_val, model.predict(x_val, verbose=0), which="validation")
    """

    return model_name


def fit_generator_model(model: Model,
                        training_data_generator, validation_data_generator,
                        n_training_data: int, n_validation_data: int,
                        params: dict[str, str | int | float | bool | list[int]],
                        monitoring: dict[str, str],
                        model_filename: str | None = None,
                        dt_string: str | None = None,
    ...
    model.fit(training_data_generator(), validation_data=validation_data_generator(), epochs=params["num_epochs"],
              batch_size=params["batch_size"], steps_per_epoch=n_training_data // params["batch_size"],
              validation_steps=n_validation_data // params["batch_size"], shuffle=True,
              callbacks=callbacks, verbose=verbose)
...
def save_model(model: Model, model_filename: str, params: dict | None = None) -> None:
    ...
    if params is not None:
        with h5py.File(model_filename, "a") as f:
            # load with ast.literal_eval(f["params"][()].decode())
...
def gimme_model_name(model_subdir: str | None, model_name: str | None) -> tuple[str, ...]:
    if model_subdir is None: model_subdir = conf_model_setup["model_subdir"]
    if model_name is None: model_name = conf_model_setup["model_name"]
...
def hp_tuner(x_train: np.ndarray, y_train: np.ndarray, x_val: np.ndarray, y_val: np.ndarray,
    ...
    if ((not used_quantities[0] or not np.any(used_quantities[1:]))
            and np.any(["_sep" in model_type for model_type in params["model_type"]])):
    ...
    order = [*metrics_all, "model_type", "num_layers", "num_residuals", *filters, "num_filters",
             ...
             "batch_size", "batch_norm_before_activation", "alpha", "c"]
    ...
    for i, (name, model) in enumerate(zip(model_names, best_model)):
        # Save top models to project dir with a timestamp
        filename = path.join(_path_model, model_subdir, name)
        save_model(model, filename, list_of_params[i])
